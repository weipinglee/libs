...
class Delivery{
	...
	public function deliveryInfoByOrder($order_id){
		$res =  empty($order_id) ? array() : $this->delivery->where(array('order_id'=>$order_id))->getObj();
		$res['num'] = number_format($res['num'],2);
		return $res;
	}
	...
	public function deliveryList($user_id,$page,$is_seller = false){
		...
		$query->where = '(('.$t.'.user_id=:user_id and off.type=1) or ('.$t2.'.user_id = :tmp_id and off.type=2)) and po.mode in ('.order\Order::ORDER_DEPOSIT.','.order\Order::ORDER_STORE.','.order\Order::ORDER_PURCHASE.','.order\Order::ORDER_FREESTORE.') and po.contract_status in ('.order\Order::CONTRACT_COMPLETE.','.order\Order::CONTRACT_ADMIN_CHECK.','.order\Order::CONTRACT_EFFECT.','.order\Order::CONTRACT_VERIFY_QAULITY.','.order\Order::CONTRACT_DELIVERY_COMPLETE.')';
			...
			switch ($value['status']) {
				...
				case self::DELIVERY_APPLY:
					if(!$is_seller){
						...
						$title = '买家申请提货';
						if($value['mode'] != order\Order::ORDER_STORE && $value['mode']!=order\Order::ORDER_FREESTORE){
							//卖家发货（保证金提货）
						...
						}elseif($value['mode'] == order\Order::ORDER_STORE){
							//支付仓库费用（仓单提货）
	...
	public function geneDelivery($deliveryData){
		...
		if(!empty($order_info)){
			...
			if($buyer == $deliveryData['user_id']){
				...
				if(isset($contract_status) && $contract_status == order\Order::CONTRACT_EFFECT && in_array($order_info['mode'],array(order\Order::ORDER_DEPOSIT,order\Order::ORDER_STORE,order\Order::ORDER_PURCHASE,order\Order::ORDER_FREESTORE))){
