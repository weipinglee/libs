...
class Order{
	...
	public function contractDetail($id,$identity = 'buyer'){
		...
		$query->fields = 'do.*,po.type,po.other,p.name,po.weight_type,po.price,do.amount,p.unit,po.product_id,po.accept_area,p.cate_id,p.img,p.produce_area,pc.name as cate_name,po.user_id as seller_id';
		$query->where = 'do.id=:id';
		$query->bind = array('id'=>$id);
		$res = $query->getObj();
